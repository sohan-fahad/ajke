# Ajke — AI Assistant Guide

## What this repo is

Ajke is a NestJS-like framework for Cloudflare Workers built on [Hono](https://hono.dev). It is a pnpm monorepo:

| Part | Location | Purpose |
|---|---|---|
| **Framework core** | `packages/core/` | Published as `@ajke/core` — decorators, DI, middleware, pipes, exceptions |
| **CLI** | `packages/cli/` | Published as `@ajke/cli` — `ajke new`, `ajke generate`, `ajke add` |
...
- **context/** — `ExecutionContext`, `ArgumentsHost`, `AppContext`/`AjkeBindings`
