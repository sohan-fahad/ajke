...
function workerTypesTemplate() {
  return `// Managed by \`ajke add\` and \`pnpm cf-typegen\`.
// Run \`pnpm ajke add kv|r2|d1|ai|...\` to add a binding — types update automatically.
...
function appContextTemplate() {
  return `import type { AppContext } from "@ajke/core";
