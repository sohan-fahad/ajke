...
function normalizeModule(ref: any): NormalizedModule {
	...
	if (typeof ref !== "function") {
		throw new Error(
			`[Ajke] Invalid module reference: ${String(ref)}. ` +
	...
	if (!config) {
		throw new Error(`[Ajke] "${ref.name}" is not a module. Did you forget the @Module() decorator?`);
	}
...
function collect(
	...
	for (const importRef of config.imports ?? []) {
		...
		if (visiting.has(importedClass)) {
			if (!isRef) {
				throw new Error(
					`[Ajke] Circular module dependency detected: "${moduleClass.name}" imports ` +
