...
function createCircularProxy(registry: Map<InjectionToken, any>, token: InjectionToken): any {
	const resolve = () => {
		...
		if (instance === undefined) {
			throw new Error(
				`[Ajke DI] Circular dependency proxy for "${tokenName(token)}" ` +
	...
	try {
		...
		if (definition === undefined || definition === actualToken) {
			if (typeof actualToken !== "function") {
				throw new Error(
					`[Ajke DI] No provider found for token "${tokenName(actualToken)}". ` +
			...
			if (instance instanceof Promise) {
				throw new Error(
					`[Ajke DI] Async factory for "${tokenName(actualToken)}" is not supported: ` +
			...
			throw new Error(
				`[Ajke DI] Invalid provider for token "${tokenName(actualToken)}": ` +
