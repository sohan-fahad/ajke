...
All notable changes to Ajke are documented here.
...
- **Logger** exported from `@ajke/core`
...
- **Middleware** — `requestLogger`, `errorHandler` (exported from `@ajke/core/middleware`)
- **CLI** — `ajke new`, `ajke generate` (module / service / controller), `ajke add` (d1, r2, kv, queue, ai, durable-object, vectorize, browser, hyperdrive)
